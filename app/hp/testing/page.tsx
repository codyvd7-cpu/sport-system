...
export default function HPTestingPage() {
  ...
  const classStudents = selectedClass ? students.filter(s => s.class_group === selectedClass) : students;
    ...
    <main className="min-h-screen bg-slate-950 pb-20 text-white md:pb-0">
      <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
        ...
        <div className="mb-5 rounded-2xl border border-slate-800 bg-slate-900 p-5">
          ...
          {/* Class selector */}
          <div className="mt-4">
            <p className="mb-2 text-[10px] font-black uppercase tracking-wide text-slate-500">Select Class</p>
            <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
              ...
              {HP_CLASSES.map(c => (
                <button key={c} onClick={() => setSelectedClass(c)}
                  className={`rounded-xl border py-2.5 text-sm font-black transition ${selectedClass === c ? 'border-violet-500/40 bg-violet-500/15 text-violet-300' : 'border-slate-700 bg-slate-800 text-slate-400 hover:text-white'}`}>
                  Class {c}
                </button>
              ))}
            </div>
