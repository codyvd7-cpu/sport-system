    ...
    <main className="min-h-screen  pb-24 text-white md:pb-0" style={{background:'#030810'}}>
      <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6">
        ...
        <div className="mb-5 rounded-2xl border border-white/6 bg-[rgba(255,255,255,0.025)] p-5">
          <p className="mb-4 text-xs font-black uppercase tracking-wide text-white/35">Step 1 — Session Details</p>
                ...
                {HP_CLASSES.map(c => {
                  ...
                  return (
                    <button key={key} onClick={() => setSelectedClass(key)}
                      className={`rounded-xl border py-2.5 text-sm font-black transition ${selectedClass === key ? 'border-sky-500/40 bg-sky-500/15 text-sky-300' : 'border-white/8 bg-white/5 text-white/50 hover:text-white'}`}>
                      {c}
                    </button>
                  );
                })}
            ...
            <div>
              ...
              <div className="grid grid-cols-5 gap-2">
                {HP_CLASSES.map(c => {
                  ...
                  return (
                    <button key={key} onClick={() => setSelectedClass(key)}
                      className={`rounded-xl border py-2.5 text-sm font-black transition ${selectedClass === key ? 'border-violet-500/40 bg-violet-500/15 text-violet-300' : 'border-white/8 bg-white/5 text-white/50 hover:text-white'}`}>
                      {c}
                    </button>
                  );
                })}
        ...
        <div className="mb-5 rounded-2xl border border-white/6 bg-[rgba(255,255,255,0.025)] p-5">
          <div className="mb-4 flex items-center justify-between">
            <p className="text-xs font-black uppercase tracking-wide text-white/35">Step 3 — Take Register ({classStudents.length} students)</p>
          ...
          <div className="space-y-1.5">
            {classStudents.map(s => (
              <div key={s.id} className="flex items-center gap-3 rounded-xl border border-white/6 bg-[rgba(255,255,255,0.02)]/40 p-3">
                ...
                <p className="text-[10px] text-white/25 mr-2 hidden sm:block">{s.class_group ? `Class ${s.class_group}` : s.grade}</p>
                <div className="flex gap-1">
                  {[{s:'Present',l:'P',a:'bg-emerald-500 text-white'},{s:'Late',l:'L',a:'bg-amber-500 text-white'},{s:'Absent',l:'A',a:'bg-red-500 text-white'},{s:'Excused',l:'E',a:'bg-sky-500 text-white'}].map(btn => (
                    <button key={btn.s} onClick={() => setStatuses(p => ({...p, [s.id]: btn.s}))}
                      className={`h-8 w-8 rounded-lg text-xs font-black transition ${statuses[s.id] === btn.s ? btn.a : 'bg-white/5 text-white/35 hover:bg-slate-700'}`}>
                      {btn.l}
                    </button>
                  ))}
                </div>
            ...
            {classStudents.length === 0 && <p className="text-sm text-white/35">No students. Add students first.</p>}
          </div>
          ...
          <div className="rounded-2xl border border-white/6 bg-[rgba(255,255,255,0.025)] p-5">
            ...
            <div className="space-y-2">
              {recentDates.map(date => {
                ...
                return (
                  <div key={date} className="flex items-center justify-between rounded-xl border border-white/6 bg-[rgba(255,255,255,0.02)]/40 p-3">
                    <div>
                      ...
                      <p className="text-[10px] text-white/35">{sessionT}</p>
                    </div>
