...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (result.error) {
        setErrorMessage(cleanAuthError(result.error.message));
        setStatus('');
        setLoading(false);
        return;
      }
      ...
      if (!result.data.session) {
        setErrorMessage('Login failed. Please confirm this user exists and is verified in Supabase.');
        setStatus('');
        setLoading(false);
        return;
      }
  ...
  if (checkingSession) {
    return (
      <main className="relative flex min-h-screen items-center justify-center bg-[#020617] px-4 text-white">
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#020617] px-4 py-8 text-white">
      ...
      <div className="absolute inset-0 bg-[linear-gradient(#ffffff_1px,transparent_1px),linear-gradient(to_right,#ffffff_1px,transparent_1px)] bg-[size:40px_40px] opacity-[0.03]" />
      ...
      <section className="relative z-10 w-full max-w-md rounded-[2rem] border border-slate-800 bg-slate-950/90 p-6 shadow-2xl backdrop-blur sm:p-8">
        <div className="mb-8 text-center">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-400">
            Secure staff access to manage the hockey portal.
          </p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full rounded-2xl bg-sky-500 px-4 py-3 text-sm font-black text-slate-950 shadow-lg shadow-sky-500/20 transition hover:bg-sky-400 disabled:cursor-not-allowed disabled:opacity-60"
          >
        ...
        <div className="mt-6 flex items-center justify-between text-xs text-slate-500">
          <a href="/portal" className="font-bold text-slate-400 transition hover:text-white">
            Back to Portal
          </a>
...
function cleanAuthError(message: string) {
  ...
  if (lower.includes('email not confirmed')) {
    return 'This account has not been confirmed yet. Confirm the user in Supabase Auth.';
  }
