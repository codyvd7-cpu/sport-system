...
export async function GET(req: NextRequest) {
  ...
  const [fixturesRes, attendanceRes, eventsRes, alert] = await Promise.all([
  ...
  // Availability: anyone not fully available is worth a coach's eye
  ...
  attention.sort((x, y) => (x.kind === y.kind ? x.name.localeCompare(y.name) : x.kind === 'attendance' ? -1 : 1));
